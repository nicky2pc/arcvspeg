...
export class Mondalak {
  ...
  width: number = 70;
  height: number = 48;  // Adjusted for better proportions
  hitboxRadius: number = 30;
  ...
  constructor(
    ...
    this.weapon = null;
    if (this.type === "fire" || this.type === "terraLuna") {
      // Luna boss is 1.5x bigger than regular enemies
      this.width = Math.floor(this.width * 1.5);
      this.height = Math.floor(this.height * 1.5);
      this.hitboxRadius = Math.floor(this.hitboxRadius * 1.3);
      this.health = 4;
      this.maxHealth = 4;
    }
  }
  ...
  drawHealthBar(ctx: CanvasRenderingContext2D) {
    ...
    ctx.save();
    const translateY = (this.type === "fire" || this.type === "terraLuna") ? 45 : 22;
