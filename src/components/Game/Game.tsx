...
import { getPlayerPowerups } from '../../game/arcMintUtils.ts';
...
const PowerupShop = React.lazy(() => import('../PowerupShop/PowerupShop.tsx'));
...
const Game = () => {
  ...
  // Character selection - Arc themed
  const characterDefs: CharacterDef[] = [
    { key: 'default', points_to_unlock: 0, name: 'Arc', src: '/chars/arc.png', moveSpeed: 2.0, health: 5, twitter: null },
    { key: 'arc_pro', points_to_unlock: 25, name: 'Arc Pro', src: '/chars/arc.png', moveSpeed: 2.3, health: 6, twitter: null },
    { key: 'arc_elite', points_to_unlock: 50, name: 'Arc Elite', src: '/chars/arc.png', moveSpeed: 2.5, health: 7, twitter: null },
    { key: 'arc_legend', points_to_unlock: 100, name: 'Arc Legend', src: '/chars/arc.png', moveSpeed: 2.7, health: 8, twitter: null },
  ];
  ...
  // Powerups from onchain
  const playerPowerupsRef = useRef({ shields: 0, speedBoosts: 0, doubles: 0 });
  const pendingDoubleDamageRef = useRef(0); // Seconds of double damage to apply when game starts
  ...
  const resetGameObjects = async () => {
    ...
    if (playerTank.current) {
      ...
      // Apply onchain powerups
      const powerups = playerPowerupsRef.current;

      // Shield: +1 health per shield owned
      if (powerups.shields > 0) {
        playerTank.current.health += powerups.shields;
        playerTank.current.maxHealth += powerups.shields;
        console.log(`Applied ${powerups.shields} shields: health now ${playerTank.current.health}`);
      }

      // Speed: +15% per speed boost owned
      if (powerups.speedBoosts > 0) {
        const speedMultiplier = 1 + (powerups.speedBoosts * 0.15);
        playerTank.current.speed *= speedMultiplier;
        console.log(`Applied ${powerups.speedBoosts} speed boosts: speed now ${playerTank.current.speed}`);
      }

      // Double: Store for activation when gameplay starts
      if (powerups.doubles > 0) {
        pendingDoubleDamageRef.current = powerups.doubles * 20;
        console.log(`Stored ${powerups.doubles} double damage: ${pendingDoubleDamageRef.current}s to activate on play`);
      } else {
        pendingDoubleDamageRef.current = 0;
      }
    }
  ...
  // Activate double damage buff when gameplay starts
  useEffect(() => {
    if (gameState === 'playing' && pendingDoubleDamageRef.current > 0 && playerTank.current) {
      const duration = pendingDoubleDamageRef.current;
      pendingDoubleDamageRef.current = 0;
      console.log(`Activating double damage buff for ${duration}s`);
      startBuffTimer(duration, playerTank);
    }
  }, [gameState]);
  ...
  const checkBalanceAndStartGame = async () => {
    ...
    // Load onchain powerups before starting
    if (authenticated && wallets[0]?.address) {
      try {
        const powerups = await getPlayerPowerups(wallets[0].address);
        playerPowerupsRef.current = {
          shields: powerups.shields,
          speedBoosts: powerups.speedBoosts,
          doubles: powerups.doubles
        };
        console.log('Loaded powerups:', playerPowerupsRef.current);
      } catch (e) {
        console.error('Failed to load powerups:', e);
        playerPowerupsRef.current = { shields: 0, speedBoosts: 0, doubles: 0 };
      }
    } else {
      playerPowerupsRef.current = { shields: 0, speedBoosts: 0, doubles: 0 };
    }
  ...
  // Arc doesn't use a centralized username system - this function is kept for compatibility
  // It will always return true (allow game start) since the wallet check returns undefined
  const checkUsernameIfNeeded = async (): Promise<boolean> => {
    if (!user) return true;
    try {
      const arcWalletAddress: string | undefined = getMonadIdWallet(user as any);
      if (!arcWalletAddress) {
        setUsername(null);
        localStorage.removeItem('arcUsername');
        localStorage.removeItem('arcUsernameWallet');
        return true;
      }

      setIsCheckingUsername(true);
      const data = await checkUsername(arcWalletAddress);
      setIsCheckingUsername(false);

      if (data?.name) {
        setUsername(data.name);
        localStorage.setItem('arcUsername', data.name);
        localStorage.setItem('arcUsernameWallet', arcWalletAddress);
        return true;
      }

      // On Arc, we don't require username - just allow game start
      return true;
    } catch (e) {
          ...
          <>
            ...
            <h1 className='total-score h1'>
              ...
              Defend the Network
                ...
                <button disabled={isStartButtonDisabled} className="leaderboard-button shop-button" onClick={() => setShowPowerupShop(true)}>
                  Shop
                </button>
                <LoginBtn />
      ...
      {gameState === 'gameover' && (
        <>
          <div className="bg">
            ...
            <button className="leaderboard-button shop-button" onClick={() => setShowPowerupShop(true)}>
              Shop
            </button>
            <LoginBtn />
            ...
            <div className="game-stat">
              <div className="row">
                <div className="col">
                  ...
                  <span>Bosses killed: {gameStat.bossesKilled}</span>
